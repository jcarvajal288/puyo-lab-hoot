...
(define (vec2-of-length angle len)
  (let ((angle-in-radians (to-radians angle)))
    (vec2 (* len (sin angle-in-radians))
          (- (* len (cos angle-in-radians)))))) ; negative because positive y is towards the bottom
